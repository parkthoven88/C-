...
/* post-condition: get the input of a year. If the date is NOT valid, will prompt the user to reenter a valid year. the year must be later than 1582 in order to be valid.*/
...
/*post-condition: print out the day of week for specified date.
                  For example, print_calendar(10, 15, 2014) will print out
    ...
    do
    {
        ...
        switch(choice)
        {
            ...
            case '4': // print out the calendar of specified year to yourname.txt file
                // you implement this ******************
                get_input(year);
                if(print_calendar(year, "Hyojin.txt"))
                   {
                    cout << "\tCalender of year "<< year <<" has be print to file Hyojin.txt";
                }
                   else
                    cout << "Failed to print out calender if year "<< year <<endl;
                break;
...
void menu()
{
    ...
    cout << "\n\t* 4. Print calender of a given year to file       *";
    cout << "\n\t* 5. Quit                                         *";
    cout << "\n\t***************************************************";
}
...
void get_input(int& month, int& day, int& year)
{
    ...
    while(!isValid(month, day, year))
        // as long as the date is NOT valid, keep asking to reenter the date
    {
...
void get_input(int& month, int& year)
{
    ...
    while(!isValid(month, year))
        // as long as the month is NOT valid, keep asking to reenter the month
    {
...
bool isValid(int month, int day, int year)
{
    ...
    //              by adding one line of comments at beginning of the file. This worth
    //              point
...
bool isValid(int month, int year)
{
    ...
    // month is between 1 and 12 and year is later than 1582
}
...
bool print_calendar(int year, string fileName)
{
   ...
   cout << "\n\tThe year is NOT valid!";
    ...
    ofstream fout;
    fout.open(fileName);
    //if fileName open failed, print out error message and return false
    if(fout.fail())
    {
        cout << "Cannot open file " << fileName << " to write\n";
        return false;
    }
...
//    Enter the year as yyyy: 2016
//    Calender of year 2016 has be print to file Hyojin.txt
